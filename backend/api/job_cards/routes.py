...
job_cards_bp = Blueprint("job_cards", __name__, url_prefix="/api/v1/job-cards")

@job_cards_bp.route("/", methods=["POST"])
@jwt_required()
@permission_required("manage_job_cards")
def create_job_card():
    ...
    return jsonify({"message": "Job card created", "job_card": job_card.to_dict()}), 201
...
def dispatch_job_card(job_card_id):
    ...
    return jsonify({"message": "Job card dispatched", "job_card": job_card.to_dict()}), 200
...
def return_job_card(job_card_id):
    ...
    try:
        job_card = JobCardService.return_items(job_card_id, data["returns"], performed_by)
    except ValueError as e:
        return jsonify({"error": str(e)}), 409
    return jsonify({"message": "Items returned successfully", "job_card": job_card.to_dict()}), 200
